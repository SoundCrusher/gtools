...
func MakeCurlBorder(_ fyne.Window) fyne.CanvasObject {
	...
	f := container.New(layout.NewGridWrapLayout(fyne.NewSize(750, 400)), left)
...
func matchCurl(curlStr string)  {
	curlStr = strings.Trim(curlStr, " ")
	//解析正则表达式，如果成功返回解释器
	reg1 := regexp.MustCompile(`-H(\s{0,10})(".*"|'.*')`)
	if reg1 == nil {
	...
	reg2 := regexp.MustCompile(`--data.*(\s{0,10})(".*"|'.*')`)
	if reg2 == nil {
		fmt.Println("regexp err")
		return
	}
	fmt.Println("data = ", reg2.FindAllString(curlStr, -1))

	// \w word
	reg3 := regexp.MustCompile(`-\w+(\s{0,10})(".*"|'.*')?`)
	if reg3 == nil {
		fmt.Println("regexp err")
		return
	}
	fmt.Println("\\w 特殊- = ", reg3.FindAllString(curlStr, -1))
}